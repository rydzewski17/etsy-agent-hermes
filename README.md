...
| 6 | `6 - Publish to Etsy` | `prepare_bundles.py` + `publish.py` | Zips files, commits them so they're fetchable, then relays to Make.com to create/upload/activate the listing |
...
`publish.py` has a placeholder `DEFAULT_TAXONOMY_ID`. Use the Make
"Make an API Call" module once to hit
`GET /v3/application/seller-taxonomy/nodes` and find the correct
node for your product category before the first real publish.
...
- **SVG preview images**: currently uploads the raw SVG as the
  listing's preview image. Etsy listing images render better as
  PNG/JPG — worth adding a rasterization step (e.g. `cairosvg`)
  before your first real batch of listings.
- **Taxonomy ID** is a placeholder, see step 4 above.
- **Repo visibility**: bundle files are served via
